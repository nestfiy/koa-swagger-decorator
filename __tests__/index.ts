        ...
        it('POST /api/v1/user/login should have corresponding swagger doc data', done => {
            request
                ...
                .expect((res: { body: { paths: { [x: string]: any } } }) =>
                    expect(res.body.paths['/api/v1/user/login']).to.be.an(
                        'object'
                    )
                )
                .end((err: any) => {
    ...
    describe('Construct api from router.mapDir:', () => {
        it('GET /api/v2/other should return data if success', done => {
            request
                .get('/api/v2/other')
                .expect(200)
                .expect((res: { body: { other: any } }) =>
                    expect(res.body.other).to.be.an('array')
                )
                .end((err: any) => {
                    if (err) return done(err)
                    done()
        ...
        it('GET /api/v2/egg/method1 should return data if success', done => {
            request
                ...
                .expect(200)
                .expect((res: { body: { no: any } }) =>
                    expect(res.body.no).to.be.an('string')
                )
                .end((err: any) => {
                    if (err) return done(err)
                    done()
        ...
        it('POST /api/v2/egg/method2 should return data if success', done => {
            request
                ...
                .expect(200)
                .expect((res: { body: { yes: any } }) =>
                    expect(res.body.yes).to.be.an('string')
                )
                .end((err: any) => {
                    if (err) return done(err)
                    done()
...
describe('Function Test:', () => {
    ...
    describe('sortObject:', () => {
        it('should return an object with keys ordered in the specified weight', () => {
            ...
            const output = sortObject(input, (item: any) => item.weight)
...
describe('Validate:', () => {
    ...
    it('should throw error when no required input', () => {
        ...
        try {
            ...
            assert(
                err.message === "incorrect field: 'foo', please check again!"
            )
        }
    })
