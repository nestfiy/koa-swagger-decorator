import Router, { RouterContext, RouterOptions } from '@koa/router'
...
const validator =
    (parameters: any) => async (ctx: Context, next: () => Promise<any>) => {
        ...
        if (parameters.query) {
            ctx.validatedQuery = validate({ ...ctx.query }, parameters.query)
        }
        if (parameters.path) {
            ctx.validatedParams = validate(ctx.params, parameters.path)
        }
        if (parameters.body) {
            ctx.validatedBody = validate(
    ...
    ;[...staticMethods, ...methods]
        ...
        .forEach(item => {
            ...
            if (
                ...
                // local query 的优先级更高
                localParams.query = Object.assign(
                    globalQuery,
                    localParams.query
                )
            }
            ...
            if (!is.array(middlewares)) {
                throw new Error(
                    'middlewares params must be an array or function'
                )
            }
